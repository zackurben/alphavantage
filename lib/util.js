...
/**
 * The available parameters for all functions.
 */
...
module.exports = config => {
   ...
   * 
   * @param {Array} params
   *   An array of arrays, each being a parameter type:value pair.
   * 
   * @returns {String}
  ...
  const url = params => {
    params = (params || [])
      .map(([type, value]) => {
        if (PARAMETER_TYPES.hasOwnProperty(type) && value !== undefined) {
          return `${type}=${value}`;
        }
